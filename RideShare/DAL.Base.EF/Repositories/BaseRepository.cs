    ...
    where TDbContext : DbContext
{
    ...
    protected IQueryable<TDomainEntity> CreateQuery(TKey? userId = default, bool noTracking = true)
    {
        ...
        if (userId != null && !userId.Equals(default) && typeof(IDomainUserId<TKey>)
    ...
    {
        if (userId != null && typeof(IDomainUserId<TKey>).IsAssignableFrom(typeof(TDomainEntity)) && !((IDomainUserId<TKey>)entity).UserId.Equals(userId))
        {
    ...
    public virtual async Task<bool> ExistsAsync(TKey id, TKey? userId = default)
    {
        ...
        if (!typeof(IDomainUserId<TKey>).IsAssignableFrom(typeof(TDomainEntity)))
            throw new AuthenticationException(
                $"Entity {typeof(TDomainEntity).Name} does not implement required interface: {typeof(IDomainUserId<TKey>).Name} for AppUserId check");
        return await RepoDbSet.AnyAsync(e =>
            e.Id.Equals(id) && ((IDomainUserId<TKey>)e).UserId.Equals(userId));
    }
}